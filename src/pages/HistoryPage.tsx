import React from 'react'
import { ConversionHistory, TierUsagePanel } from '@components/application'

const HistoryPage: React.FC = () => {
  const mockConversions = [
    {
      id: '1',
      title: 'Blog post about AI trends',
      status: 'success' as const,
      platform: 'Twitter',
      platformIcon: '🐦',
      originalLength: 1250,
      convertedLength: 280,
      date: '2025-04-14',
      time: '14:30',
      creditsUsed: 5,
    },
    {
      id: '2',
      title: 'Product announcement draft',
      status: 'pending' as const,
      platform: 'LinkedIn',
      platformIcon: '💼',
      originalLength: 800,
      convertedLength: 300,
      date: '2025-04-13',
      time: '10:15',
      creditsUsed: 4,
    },
    {
      id: '3',
      title: 'Marketing campaign content',
      status: 'success' as const,
      platform: 'Instagram',
      platformIcon: '📸',
      originalLength: 600,
      convertedLength: 220,
      date: '2025-04-12',
      time: '16:45',
      creditsUsed: 3,
    },
    {
      id: '4',
      title: 'Event promotion text',
      status: 'failed' as const,
      platform: 'Facebook',
      platformIcon: '📘',
      originalLength: 900,
      convertedLength: 0,
      date: '2025-04-11',
      time: '09:20',
      creditsUsed: 2,
    },
  ]

  const mockStats = {
    totalConversions: 42,
    creditsUsed: 156,
    avgConversionTime: '1.2s',
    successRate: 85,
    change: {
      value: 12,
      direction: 'positive' as const,
    },
  }

  const mockUpgradeOptions = [
    {
      id: 'pro',
      title: 'Pro Plan',
      price: '$29',
      period: 'month',
      features: [
        { id: 'f1', text: '10,000 credits/month' },
        { id: 'f2', text: 'All AI models' },
        { id: 'f3', text: 'Priority support' },
        { id: 'f4', text: 'Advanced analytics' },
      ],
      recommended: true,
      buttonText: 'Upgrade Now',
    },
    {
      id: 'enterprise',
      title: 'Enterprise',
      price: '$99',
      period: 'month',
      features: [
        { id: 'f1', text: 'Unlimited credits' },
        { id: 'f2', text: 'Custom AI models' },
        { id: 'f3', text: 'Dedicated support' },
        { id: 'f4', text: 'Team collaboration' },
      ],
      buttonText: 'Contact Sales',
    },
  ]

  return (
    <div>
      <ConversionHistory conversions={mockConversions} stats={mockStats} />
      <div style={{ marginTop: 'var(--sys-spacing-2xl)' }}>
        <TierUsagePanel
          currentPlan={{
            title: 'Pro Plan',
            badge: 'Active',
            description:
              'Your current plan includes 10,000 credits per month with access to all AI models and priority support.',
            features: [
              { id: '1', text: '10,000 credits/month' },
              { id: '2', text: 'All AI models (GPT-4, Claude 3, Gemini)' },
              { id: '3', text: 'Priority email support' },
              { id: '4', text: 'Advanced analytics dashboard' },
            ],
          }}
          usageStats={[
            {
              id: 'credits',
              title: 'Credits Used',
              value: '1,234',
              label: 'of 10,000 total',
              progress: 65,
            },
            {
              id: 'conversions',
              title: 'Conversions',
              value: '42',
              label: 'this month',
              progress: 42,
            },
            { id: 'success', title: 'Success Rate', value: '85%', label: 'average', progress: 85 },
          ]}
          upgradeOptions={mockUpgradeOptions}
          billingInfo={{
            nextBillingDate: '2025-05-14',
            paymentMethod: 'Visa **** 4242',
            billingEmail: 'john@example.com',
          }}
        />
      </div>
    </div>
  )
}

export default HistoryPage
